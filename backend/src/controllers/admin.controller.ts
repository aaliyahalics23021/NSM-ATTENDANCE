...
export async function exportAttendanceExcel(req: Request, res: Response) {
  try {
    ...
    let currentYear = parseInt(istYear, 10);
    let currentMonth = parseInt(istMonthStr, 10) - 1; // 0-11

    if (req.query.month) {
      const parsedMonth = parseInt(String(req.query.month), 10);
      if (!isNaN(parsedMonth) && parsedMonth >= 1 && parsedMonth <= 12) {
        currentMonth = parsedMonth - 1; // Convert 1-12 to 0-11
      }
    }
    if (req.query.year) {
    ...
    employees.forEach((emp) => {
      ...
      const isPastMonth =
        currentYear < todayYearIST ||
        (currentYear === todayYearIST && currentMonth < todayMonthIST);
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        const isPastDay = isPastMonth || (isCurrentMonth && day < todayDayNum);
